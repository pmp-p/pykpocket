...
inline bool
is_identifier (std::string_view s)
{
  if (s.empty ())
    return false;
...
int
CodeEmitContext::get_loop () const
{
...
CodeBlock *
CodeEmitContext::enter_block (CodeBlockType type)
{
  if (type == CodeBlockType::FOR_LOOP
      || type == CodeBlockType::CONTEXT_MANAGER)
    base_stack_size++;
  co->blocks.push_back (
      CodeBlock (type, curr_block_i, base_stack_size, (int)co->codes.size ()));
  curr_block_i = co->blocks.size () - 1;
  return &co->blocks[curr_block_i];
}

void
CodeEmitContext::exit_block ()
{
  auto curr_type = co->blocks[curr_block_i].type;
  if (curr_type == CodeBlockType::FOR_LOOP
      || curr_type == CodeBlockType::CONTEXT_MANAGER)
    base_stack_size--;
...
int
CodeEmitContext::emit_ (Opcode opcode, uint16_t arg, int line)
{
...
void
CodeEmitContext::patch_jump (int index)
{
...
bool
CodeEmitContext::add_label (StrName name)
{
...
int
CodeEmitContext::add_varname (StrName name)
{
...
int
CodeEmitContext::add_const_string (std::string_view key)
{
...
int
CodeEmitContext::add_const (PyObject *v)
{
...
int
CodeEmitContext::add_func_decl (FuncDecl_ decl)
{
...
void
CodeEmitContext::emit_store_name (NameScope scope, StrName name, int line)
{
  switch (scope)
    {
    case NAME_LOCAL:
...
void
NameExpr::emit_ (CodeEmitContext *ctx)
{
  ...
  if (scope == NAME_LOCAL && index >= 0)
  ...
  else
    {
      ...
      if (ctx->is_compiling_class && scope == NAME_GLOBAL)
        {
          // if we are compiling a class, we should use OP_LOAD_ATTR_GLOBAL
          // instead of OP_LOAD_GLOBAL this supports @property.setter
...
bool
NameExpr::emit_del (CodeEmitContext *ctx)
{
  switch (scope)
    {
    case NAME_LOCAL:
...
bool
NameExpr::emit_store (CodeEmitContext *ctx)
{
...
void
InvertExpr::emit_ (CodeEmitContext *ctx)
{
  child->emit_ (ctx);
...
void
StarredExpr::emit_ (CodeEmitContext *ctx)
{
  child->emit_ (ctx);
...
bool
StarredExpr::emit_store (CodeEmitContext *ctx)
{
  if (level != 1)
    return false;
...
void
NotExpr::emit_ (CodeEmitContext *ctx)
{
  child->emit_ (ctx);
...
void
AndExpr::emit_ (CodeEmitContext *ctx)
{
  lhs->emit_ (ctx);
...
void
OrExpr::emit_ (CodeEmitContext *ctx)
{
  lhs->emit_ (ctx);
...
void
Literal0Expr::emit_ (CodeEmitContext *ctx)
{
  switch (token)
    {
    case TK ("None"):
...
void
LongExpr::emit_ (CodeEmitContext *ctx)
{
...
void
ImagExpr::emit_ (CodeEmitContext *ctx)
{
  VM *vm = ctx->vm;
...
void
BytesExpr::emit_ (CodeEmitContext *ctx)
{
  ctx->emit_ (OP_LOAD_CONST, ctx->add_const_string (s.sv ()), line);
...
void
LiteralExpr::emit_ (CodeEmitContext *ctx)
{
  VM *vm = ctx->vm;
...
void
NegatedExpr::emit_ (CodeEmitContext *ctx)
{
  VM *vm = ctx->vm;
...
void
SliceExpr::emit_ (CodeEmitContext *ctx)
{
  if (start)
    {
      start->emit_ (ctx);
...
void
DictItemExpr::emit_ (CodeEmitContext *ctx)
{
  if (is_starred ())
    {
...
bool
TupleExpr::emit_store (CodeEmitContext *ctx)
{
  // TOS is an iterable
...
bool
TupleExpr::emit_del (CodeEmitContext *ctx)
{
  for (auto &e : items)
    {
...
void
CompExpr::emit_ (CodeEmitContext *ctx)
{
...
void
FStringExpr::_load_simple_expr (CodeEmitContext *ctx, Str expr)
{
  bool repr = false;
...
void
FStringExpr::emit_ (CodeEmitContext *ctx)
{
  int i = 0;         // left index
  int j = 0;         // right index
  ...
  const char *fmt_valid_chars = "0-=*#@!~"
                                ...
                                ".fds"
                                "0123456789";
  PK_LOCAL_STATIC const std::set<char> fmt_valid_char_set (
      fmt_valid_chars, fmt_valid_chars + strlen (fmt_valid_chars));
                  ...
                  if (ok)
                    {
                      ...
                      ctx->emit_ (OP_FORMAT_STRING,
                                  ctx->add_const_string (spec.sv ()), line);
                    }
                  else
                    {
      ...
      else
        {
          if (src[j] == '{')
              ...
              if (j + 1 < src.size && src[j + 1] == '{')
                {
                  // {{ -> {
                  j++;
                  ctx->emit_ (OP_LOAD_CONST, ctx->add_const_string ("{"),
                              line);
                  count++;
                }
              else
          ...
          else
            {
              ...
              ctx->emit_ (OP_LOAD_CONST, ctx->add_const_string (literal.sv ()),
                          line);
              count++;
              continue; // skip j++
            }
...
void
SubscrExpr::emit_ (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
  b->emit_ (ctx);
...
bool
SubscrExpr::emit_del (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
  b->emit_ (ctx);
...
bool
SubscrExpr::emit_store (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
  b->emit_ (ctx);
...
void
AttribExpr::emit_ (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
...
bool
AttribExpr::emit_del (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
  int index = StrName (b).index;
...
bool
AttribExpr::emit_store (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
  int index = StrName (b).index;
...
void
AttribExpr::emit_method (CodeEmitContext *ctx)
{
  a->emit_ (ctx);
  int index = StrName (b).index;
...
void
CallExpr::emit_ (CodeEmitContext *ctx)
{
  bool vargs = false;
  bool vkwargs = false;
  for (auto &arg : args)
  ...
  // if callable is a AttrExpr, we should try to use `fast_call` instead of use
  // `boundmethod` proxy
...
void
BinaryExpr::_emit_compare (CodeEmitContext *ctx, std::vector<int> &jmps)
{
  if (lhs->is_compare ())
    {
...
void
BinaryExpr::emit_ (CodeEmitContext *ctx)
{
  std::vector<int> jmps;
...
void
TernaryExpr::emit_ (CodeEmitContext *ctx)
{
  cond->emit_ (ctx);
