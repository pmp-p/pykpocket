...
{
PyObject **
FastLocals::try_get_name (StrName name)
{
...
PyObject *
Frame::f_closure_try_get (StrName name)
{
...
bool
Frame::jump_to_exception_handler ()
{
  // try to find a parent try block
  int block = co->iblocks[_ip];
  while (block >= 0)
    {
      if (co->blocks[block].type == CodeBlockType::TRY_EXCEPT)
        break;
  ...
  if (stack_size () < _stack_size)
    throw std::runtime_error (
        fmt ("invalid state: ", stack_size (), '<', _stack_size).str ());
  _s->reset (actual_sp_base () + _locals.size ()
             + _stack_size); // rollback the stack
...
void
Frame::jump_abs_break (int target)
{
