...
void
RangeIter::_register (VM *vm, PyObject *mod, PyObject *type)
{
  ...
  vm->bind__next__ (PK_OBJ_GET (Type, type), [] (VM *vm, PyObject *obj) {
    ...
    bool has_next = self.r.step > 0 ? self.current < self.r.stop
                                    : self.current > self.r.stop;
    if (!has_next)
...
void
ArrayIter::_register (VM *vm, PyObject *mod, PyObject *type)
{
  vm->_all_types[PK_OBJ_GET (Type, type)].subclass_enabled = false;
...
void
StringIter::_register (VM *vm, PyObject *mod, PyObject *type)
{
  vm->_all_types[PK_OBJ_GET (Type, type)].subclass_enabled = false;
  ...
  if (ret == PY_OP_YIELD)
    {
      ...
      ret = frame._s->popx ();
      for (PyObject *obj : frame.stack_view ())
...
void
Generator::_register (VM *vm, PyObject *mod, PyObject *type)
{
  vm->_all_types[PK_OBJ_GET (Type, type)].subclass_enabled = false;
...
PyObject *
VM::_py_generator (Frame &&frame, ArgsView buffer)
{
