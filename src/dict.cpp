...
Dict::Dict (VM *vm)
    : vm (vm), _capacity (__Capacity), _mask (__Capacity - 1), _size (0),
      _critical_size (__Capacity * __LoadFactor + 0.5f), _head_idx (-1),
      _tail_idx (-1)
{
...
void
Dict::set (PyObject *key, PyObject *val)
{
  // do possible rehash
...
PyObject *
Dict::try_get (PyObject *key) const
{
  bool ok;
  int i;
  _probe_0 (key, ok, i);
...
bool
Dict::contains (PyObject *key) const
{
  bool ok;
  int i;
  _probe_0 (key, ok, i);
...
Dict::erase (PyObject *key)
{
  ...
  // _items[i].second = PY_DELETED_SLOT;  // do not change .second if it is not
  // NULL, it means the slot is occupied by a deleted item
  _size--;
