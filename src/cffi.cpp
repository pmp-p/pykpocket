...
void
VoidP::_register (VM *vm, PyObject *mod, PyObject *type)
{
...
#define BIND_CMP(name, op)                                                    \
  vm->bind##name (PK_OBJ_GET (Type, type),                                    \
                  [] (VM *vm, PyObject *lhs, PyObject *rhs) {                 \
                    if (!vm->isinstance (rhs, VoidP::_type (vm)))             \
...
void
C99Struct::_register (VM *vm, PyObject *mod, PyObject *type)
{
  vm->bind_constructor<2> (type, [] (VM *vm, ArgsView args) {
    Type cls = PK_OBJ_GET (Type, args[0]);
        ...
        for (int i = 0; i < s.size; i += 2)
          {
            ...
            if (s[i] >= '0' && s[i] <= '9')
              ...
              vm->ValueError (fmt ("invalid hex char: '", s[i], "'"));
            c <<= 4;
            if (s[i + 1] >= '0' && s[i + 1] <= '9')
              c += s[i + 1] - '0';
              ...
              vm->ValueError (fmt ("invalid hex char: '", s[i + 1], "'"));
            buffer.p[i / 2] = c;
          }
  ...
  vm->bind__eq__ (PK_OBJ_GET (Type, type),
                  [] (VM *vm, PyObject *lhs, PyObject *rhs) {
                    C99Struct &self = _CAST (C99Struct &, lhs);
                    if (!is_non_tagged_type (rhs, C99Struct::_type (vm)))
                      return vm->NotImplemented;
                    C99Struct &other = _CAST (C99Struct &, rhs);
                    bool ok = self.size == other.size
                              && memcmp (self.p, other.p, self.size) == 0;
                    return VAR (ok);
                  });

#define BIND_SETGET(T, name)                                                  \
  vm->bind (type, "read_" name "(self, offset=0)",                            \
            [] (VM *vm, ArgsView args) {                                      \
              C99Struct &self = _CAST (C99Struct &, args[0]);                 \
  ...
  vm->bind (type, "write_" name "(self, value, offset=0)",                    \
            [] (VM *vm, ArgsView args) {                                      \
              C99Struct &self = _CAST (C99Struct &, args[0]);                 \
...
add_module_c (VM *vm)
{
  ...
  vm->bind (mod, "p_cast(ptr: 'void_p', cls: type[T]) -> T",
            [] (VM *vm, ArgsView args) {
...
#define BIND_PRIMITIVE(T, CNAME)                                              \
  ...
  vm->bind__setitem__ (                                                       \
      type_t, [] (VM *vm, PyObject *obj, PyObject *index, PyObject *value) {  \
        VoidP &voidp = PK_OBJ_GET (VoidP, obj);                               \
        i64 offset = CAST (i64, index);                                       \
        T *target = (T *)voidp.ptr;                                           \
...
#undef BIND_PRIMITIVE
  ...
  vm->bind (char_p_t, "write_string(self, value: str)",
            [] (VM *vm, ArgsView args) {
              VoidP &voidp = PK_OBJ_GET (VoidP, args[0]);
