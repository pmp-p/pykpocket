...
SourceData::SourceData (const Str &source, const Str &filename,
                        CompileMode mode)
    : filename (filename), mode (mode)
{
  int index = 0;
  // Skip utf8 BOM if there is any.
  if (strncmp (source.begin (), "\xEF\xBB\xBF", 3) == 0)
    index += 3;
  // Drop all '\r'
  SStream ss;
...
SourceData::SourceData (const Str &filename, CompileMode mode)
    : filename (filename), mode (mode)
{
  line_starts.push_back (this->source.c_str ());
}
...
SourceData::_get_line (int lineno) const
{
  if (lineno == -1)
...
Str
SourceData::snapshot (int lineno, const char *cursor,
  ...
  if (!source.empty ())
    {
      ...
      ss << "    " << line;
      if (cursor && line != "<?>" && cursor >= pair.first
          && cursor <= pair.second)
        {
