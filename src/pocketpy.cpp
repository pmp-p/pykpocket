...
#define BIND_NUM_LOGICAL_OPT(name, op)                                        \
  _vm->bind##name (VM::tp_int, [] (VM *vm, PyObject *lhs, PyObject *rhs) {    \
    i64 val;                                                                  \
    if (try_cast_int (rhs, &val))                                             \
      return VAR (_CAST (i64, lhs) op val);                                   \
    if (is_float (rhs))                                                       \
      return VAR (_CAST (i64, lhs) op _CAST (f64, rhs));                      \
    return vm->NotImplemented;                                                \
  });                                                                         \
  _vm->bind##name (VM::tp_float, [] (VM *vm, PyObject *lhs, PyObject *rhs) {  \
    i64 val;                                                                  \
    if (try_cast_int (rhs, &val))                                             \
      return VAR (_CAST (f64, lhs) op val);                                   \
    if (is_float (rhs))                                                       \
      return VAR (_CAST (f64, lhs) op _CAST (f64, rhs));                      \
    return vm->NotImplemented;                                                \
  });
...
#undef BIND_NUM_LOGICAL_OPT
    ...
    if (args.size () == 2)
    ...
    else if (args.size () == 0)
      {
        ...
        if (class_arg == nullptr || self_arg == nullptr)
          {
            vm->TypeError ("super(): unable to determine the class context, "
                           "use super(class, self) instead");
          }
      }
    else
      {
    ...
    if (!vm->isinstance (self_arg, type))
      {
        ...
        vm->TypeError ("super(): " + _0.escape () + " is not an instance of "
                       + _1.escape ());
      }
    return vm->heap.gcnew<Super> (vm->tp_super, self_arg,
                                  vm->_all_types[type].base);
  });

  _vm->bind_func<1> (
      _vm->builtins, "staticmethod", [] (VM *vm, ArgsView args) {
        ...
        for (PyObject *type : types)
          {
            ...
            if (vm->isinstance (args[0], PK_OBJ_GET (Type, type)))
              return vm->True;
          }
        return vm->False;
      }
  ...
  _vm->bind_func<2> (_vm->builtins, "issubclass", [] (VM *vm, ArgsView args) {
    ...
    vm->check_non_tagged_type (args[1], vm->tp_type);
    return VAR (vm->issubclass (PK_OBJ_GET (Type, args[0]),
                                PK_OBJ_GET (Type, args[1])));
  });
  ...
  _vm->bind (_vm->builtins, "round(x, ndigits=0)", [] (VM *vm, ArgsView args) {
    ...
    if (x >= 0)
      {
        return VAR ((i64)(x * std::pow (10, ndigits) + 0.5)
                    / std::pow (10, ndigits));
      }
    else
      {
  ...
  _vm->bind (_vm->builtins, "eval(__source, __globals=None)",
             [] (VM *vm, ArgsView args) {
               CodeObject_ code = vm->compile (CAST (Str &, args[0]), "<eval>",
                                               EVAL_MODE, true);
               PyObject *globals = args[1];
               if (globals == vm->None)
                 {
                   FrameId frame = vm->top_frame ();
                   return vm->_exec (code.get (), frame->_module,
                                     frame->_callable, frame->_locals);
                 }
  ...
  _vm->bind (_vm->builtins, "exec(__source, __globals=None)",
             [] (VM *vm, ArgsView args) {
               CodeObject_ code = vm->compile (CAST (Str &, args[0]), "<exec>",
                                               EXEC_MODE, true);
               PyObject *globals = args[1];
               if (globals == vm->None)
                 {
                   FrameId frame = vm->top_frame ();
                   vm->_exec (code.get (), frame->_module, frame->_callable,
                              frame->_locals);
                   return vm->None;
                 }
               vm->check_non_tagged_type (globals, vm->tp_mappingproxy);
  ...
  _vm->bind_func<2> (_vm->builtins, "hasattr", [] (VM *vm, ArgsView args) {
    return VAR (vm->getattr (args[0], CAST (Str &, args[1]), false)
                != nullptr);
  });
  ...
  _vm->bind_func<1> (_vm->builtins, "dir", [] (VM *vm, ArgsView args) {
    ...
    names.insert (keys.begin (), keys.end ());
    List ret;
    for (StrName name : names)
      ret.push_back (VAR (name.sv ()));
  ...
  _vm->cached_object__new__ = _vm->bind_constructor<1> (
      _vm->_t (VM::tp_object), [] (VM *vm, ArgsView args) {
        vm->check_non_tagged_type (args[0], vm->tp_type);
  ...
  _vm->bind_method<0> (
      VM::tp_object, "_enable_instance_dict", [] (VM *vm, ArgsView args) {
  ...
  // tp_type
  _vm->bind_constructor<2> (_vm->_t (VM::tp_type),
                            PK_LAMBDA (vm->_t (args[1])));

  // tp_range
  _vm->bind_constructor<-1> (
      _vm->_t (VM::tp_range), [] (VM *vm, ArgsView args) {
        ...
        switch (args.size ())
          ...
          case 3:
            ...
            break;
          default:
            vm->TypeError ("expected 1-3 arguments, got "
                           + std::to_string (args.size ()));
          }
        return VAR (r);
      });

  _vm->bind__iter__ (VM::tp_range, [] (VM *vm, PyObject *obj) {
    return VAR_T (RangeIter, PK_OBJ_GET (Range, obj));
  });

  // tp_nonetype
  _vm->bind__repr__ (_vm->_tp (_vm->None),
                     [] (VM *vm, PyObject *obj) { return VAR ("None"); });

  // tp_float / tp_float
  _vm->bind__truediv__ (VM::tp_float,
                        [] (VM *vm, PyObject *lhs, PyObject *rhs) {
  ...
  _vm->bind_constructor<-1> (_vm->_t (VM::tp_int), [] (VM *vm, ArgsView args) {
    ...
    if (args.size () == 1 + 1)
      {
        if (is_type (args[1], vm->tp_float))
          return VAR ((i64)CAST (f64, args[1]));
        if (is_type (args[1], vm->tp_int))
          return args[1];
        if (is_type (args[1], vm->tp_bool))
          return VAR (_CAST (bool, args[1]) ? 1 : 0);
      }
    if (args.size () > 1 + 2)
  ...
  _vm->bind__floordiv__ (VM::tp_int,
                         [] (VM *vm, PyObject *lhs_, PyObject *rhs_) {
  ...
  _vm->bind__repr__ (VM::tp_int, [] (VM *vm, PyObject *obj) {
    return VAR (std::to_string (_CAST (i64, obj)));
  });
  _vm->bind__neg__ (VM::tp_int, [] (VM *vm, PyObject *obj) {
    return VAR (-_CAST (i64, obj));
  });
  _vm->bind__hash__ (VM::tp_int,
                     [] (VM *vm, PyObject *obj) { return _CAST (i64, obj); });
  _vm->bind__invert__ (VM::tp_int, [] (VM *vm, PyObject *obj) {
    return VAR (~_CAST (i64, obj));
  });
...
#undef INT_BITWISE_OP
  ...
  _vm->bind_constructor<-1> (
      _vm->_t (VM::tp_float), [] (VM *vm, ArgsView args) {
        if (args.size () == 1 + 0)
          return VAR (0.0);
        if (args.size () > 1 + 1)
        ...
        if (is_type (args[1], vm->tp_float))
          return args[1];
        if (is_type (args[1], vm->tp_bool))
          return VAR (_CAST (bool, args[1]) ? 1.0 : 0.0);
        if (is_type (args[1], vm->tp_str))
          {
            const Str &s = CAST (Str &, args[1]);
            if (s == "inf")
  ...
  _vm->bind__neg__ (VM::tp_float, [] (VM *vm, PyObject *obj) {
    return VAR (-_CAST (f64, obj));
  });
  ...
  // tp_str
  _vm->bind_constructor<2> (_vm->_t (VM::tp_str),
                            PK_LAMBDA (vm->py_str (args[1])));
  ...
  _vm->bind__contains__ (VM::tp_str,
                         [] (VM *vm, PyObject *lhs, PyObject *rhs) {
                           const Str &self = _CAST (Str &, lhs);
                           return VAR (self.index (CAST (Str &, rhs)) != -1);
                         });
  _vm->bind__str__ (VM::tp_str, [] (VM *vm, PyObject *obj) { return obj; });
  _vm->bind__iter__ (VM::tp_str, [] (VM *vm, PyObject *obj) {
    return VAR_T (StringIter, obj);
  });
...
#define BIND_CMP_STR(name, op)                                                \
  _vm->bind##name (VM::tp_str, [] (VM *vm, PyObject *lhs, PyObject *rhs) {    \
    if (!is_non_tagged_type (rhs, vm->tp_str))                                \
      return vm->NotImplemented;                                              \
...
#undef BIND_CMP_STR
  ...
  _vm->bind__getitem__ (
      VM::tp_str, [] (VM *vm, PyObject *obj, PyObject *index) {
        const Str &self = _CAST (Str &, obj);
  ...
  _vm->bind (_vm->_t (VM::tp_str), "replace(self, old, new, count=-1)",
             [] (VM *vm, ArgsView args) {
               const Str &self = _CAST (Str &, args[0]);
  ...
  _vm->bind (_vm->_t (VM::tp_str), "split(self, sep=' ')",
             [] (VM *vm, ArgsView args) {
               const Str &self = _CAST (Str &, args[0]);
  ...
  _vm->bind (_vm->_t (VM::tp_str), "splitlines(self)",
             [] (VM *vm, ArgsView args) {
               const Str &self = _CAST (Str &, args[0]);
               std::vector<std::string_view> parts;
               parts = self.split ('\n');
  ...
  _vm->bind (_vm->_t (VM::tp_str), "count(self, s: str)",
             [] (VM *vm, ArgsView args) {
               const Str &self = _CAST (Str &, args[0]);
               const Str &s = CAST (Str &, args[1]);
  ...
  // tp_list / tp_tuple
  _vm->bind (_vm->_t (VM::tp_list), "sort(self, key=None, reverse=False)",
             [] (VM *vm, ArgsView args) {
               List &self = _CAST (List &, args[0]);
               PyObject *key = args[1];
               if (key == vm->None)
                 {
                   std::stable_sort (self.begin (), self.end (),
                                     [vm] (PyObject *a, PyObject *b) {
                                       return vm->py_lt (a, b);
                                     });
                 }
               else
                 {
                   std::stable_sort (self.begin (), self.end (),
                                     [vm, key] (PyObject *a, PyObject *b) {
                                       return vm->py_lt (vm->call (key, a),
                                                         vm->call (key, b));
                                     });
                 }
  ...
  _vm->bind_constructor<-1> (_vm->_t (VM::tp_list),
                             [] (VM *vm, ArgsView args) {
                               if (args.size () == 1 + 0)
                                 return VAR (List ());
                               if (args.size () == 1 + 1)
  ...
  _vm->bind__contains__ (VM::tp_list,
                         [] (VM *vm, PyObject *obj, PyObject *item) {
  ...
  _vm->bind__eq__ (VM::tp_list, [] (VM *vm, PyObject *lhs, PyObject *rhs) {
    ...
    if (!is_non_tagged_type (rhs, vm->tp_list))
      return vm->NotImplemented;
  ...
  _vm->bind_method<0> (VM::tp_list, "copy",
                       PK_LAMBDA (VAR (_CAST (List, args[0]))));

#define BIND_RICH_CMP(name, op, _t, _T)                                       \
  _vm->bind__##name##__ (_vm->_t, [] (VM *vm, PyObject *lhs, PyObject *rhs) { \
    if (!is_non_tagged_type (rhs, vm->_t))                                    \
      return vm->NotImplemented;                                              \
    auto &a = _CAST (_T &, lhs);                                              \
    auto &b = _CAST (_T &, rhs);                                              \
...
#undef BIND_RICH_CMP
  ...
  _vm->bind__setitem__ (VM::tp_list, [] (VM *vm, PyObject *obj,
  ...
  _vm->bind__delitem__ (VM::tp_list,
                        [] (VM *vm, PyObject *obj, PyObject *index) {
                          List &self = _CAST (List &, obj);
                          int i = CAST (int, index);
                          i = vm->normalized_index (i, self.size ());
                          self.erase (i);
  ...
  _vm->bind_constructor<-1> (
      _vm->_t (VM::tp_tuple), [] (VM *vm, ArgsView args) {
        if (args.size () == 1 + 0)
          return VAR (Tuple (0));
        if (args.size () == 1 + 1)
          {
  ...
  _vm->bind__contains__ (VM::tp_tuple,
                         [] (VM *vm, PyObject *obj, PyObject *item) {
  ...
  _vm->bind__eq__ (VM::tp_tuple, [] (VM *vm, PyObject *lhs, PyObject *rhs) {
    ...
    if (!is_non_tagged_type (rhs, vm->tp_tuple))
      return vm->NotImplemented;
  ...
  // tp_bool
  _vm->bind_constructor<2> (_vm->_t (VM::tp_bool),
                            PK_LAMBDA (VAR (vm->py_bool (args[1]))));
  ...
  _vm->bind__eq__ (VM::tp_bool, [] (VM *vm, PyObject *lhs, PyObject *rhs) {
    if (is_non_tagged_type (rhs, vm->tp_bool))
      return VAR (lhs == rhs);
    if (is_int (rhs))
      return VAR (_CAST (bool, lhs) == (bool)CAST (i64, rhs));
    return vm->NotImplemented;
  });

  // tp_ellipsis / tp_NotImplementedType
  _vm->bind__repr__ (_vm->_tp (_vm->Ellipsis),
                     [] (VM *vm, PyObject *self) { return VAR ("..."); });
  _vm->bind__repr__ (
      _vm->_tp (_vm->NotImplemented),
      [] (VM *vm, PyObject *self) { return VAR ("NotImplemented"); });

  // tp_bytes
  _vm->bind_constructor<2> (
      _vm->_t (VM::tp_bytes), [] (VM *vm, ArgsView args) {
  ...
  _vm->bind__getitem__ (VM::tp_bytes,
                        [] (VM *vm, PyObject *obj, PyObject *index) {
  ...
  _vm->bind__eq__ (VM::tp_bytes, [] (VM *vm, PyObject *lhs, PyObject *rhs) {
    if (!is_non_tagged_type (rhs, vm->tp_bytes))
      return vm->NotImplemented;
    return VAR (_CAST (Bytes &, lhs) == _CAST (Bytes &, rhs));
  });

  // tp_slice
  _vm->bind_constructor<4> (_vm->_t (VM::tp_slice),
                            [] (VM *vm, ArgsView args) {
  ...
  // tp_mappingproxy
  _vm->bind_method<0> (VM::tp_mappingproxy, "keys",
                       [] (VM *vm, ArgsView args) {
  ...
  _vm->bind_method<0> (VM::tp_mappingproxy, "values",
                       [] (VM *vm, ArgsView args) {
                         MappingProxy &self = _CAST (MappingProxy &, args[0]);
                         List values;
  ...
  _vm->bind_method<
      0> (VM::tp_mappingproxy, "items", [] (VM *vm, ArgsView args) {
    MappingProxy &self = _CAST (MappingProxy &, args[0]);
    List items;
    for (auto &item : self.attr ().items ())
      {
  ...
  _vm->bind__eq__ (VM::tp_mappingproxy,
                   [] (VM *vm, PyObject *obj, PyObject *other) {
  ...
  _vm->bind__getitem__ (
      VM::tp_mappingproxy, [] (VM *vm, PyObject *obj, PyObject *index) {
  ...
  _vm->bind (_vm->_t (VM::tp_mappingproxy), "get(self, key, default=None)",
             [] (VM *vm, ArgsView args) {
               MappingProxy &self = _CAST (MappingProxy &, args[0]);
    ...
    for (auto &item : self.attr ().items ())
      {
        ...
        ss << item.first.escape () << ": "
           << CAST (Str, vm->py_repr (item.second));
      }
    ss << "})";
    return VAR (ss.str ());
  });

  _vm->bind__contains__ (
      VM::tp_mappingproxy, [] (VM *vm, PyObject *obj, PyObject *key) {
        MappingProxy &self = _CAST (MappingProxy &, obj);
  ...
  _vm->bind_constructor<-1> (
      _vm->_t (VM::tp_dict),
      [] (VM *vm, ArgsView args) { return VAR (Dict (vm)); });
        ...
        for (PyObject *item : list)
          {
            ...
            if (t.size () != 2)
              {
                vm->ValueError (
                    "dict() takes an iterable of tuples (key, value)");
                return vm->None;
              }
            self.set (t[0], t[1]);
          }
  ...
  _vm->bind__getitem__ (VM::tp_dict,
                        [] (VM *vm, PyObject *obj, PyObject *index) {
  ...
  _vm->bind__setitem__ (
      VM::tp_dict, [] (VM *vm, PyObject *obj, PyObject *key, PyObject *value) {
        Dict &self = _CAST (Dict &, obj);
        self.set (key, value);
      });

  _vm->bind__delitem__ (VM::tp_dict,
                        [] (VM *vm, PyObject *obj, PyObject *key) {
                          Dict &self = _CAST (Dict &, obj);
  ...
  _vm->bind__contains__ (VM::tp_dict,
                         [] (VM *vm, PyObject *obj, PyObject *key) {
                           Dict &self = _CAST (Dict &, obj);
  ...
  _vm->bind__eq__ (VM::tp_dict, [] (VM *vm, PyObject *a, PyObject *b) {
    ...
    if (!is_non_tagged_type (b, vm->tp_dict))
      return vm->NotImplemented;
  ...
  // tp_property
  _vm->bind_constructor<-1> (
      _vm->_t (VM::tp_property), [] (VM *vm, ArgsView args) {
        if (args.size () == 1 + 1)
          {
  ...
  // properties
  _vm->bind_property (_vm->_t (VM::tp_property), "__signature__",
                      [] (VM *vm, ArgsView args) {
                        Property &self = _CAST (Property &, args[0]);
                        return VAR (self.signature);
                      });

  _vm->bind_property (_vm->_t (VM::tp_function), "__doc__",
                      [] (VM *vm, ArgsView args) {
                        Function &func = _CAST (Function &, args[0]);
                        return VAR (func.decl->docstring);
                      });

  _vm->bind_property (_vm->_t (VM::tp_native_func), "__doc__",
                      [] (VM *vm, ArgsView args) {
  ...
  _vm->bind_property (_vm->_t (VM::tp_function), "__signature__",
                      [] (VM *vm, ArgsView args) {
                        Function &func = _CAST (Function &, args[0]);
                        return VAR (func.decl->signature);
                      });

  _vm->bind_property (_vm->_t (VM::tp_native_func), "__signature__",
                      [] (VM *vm, ArgsView args) {
                        NativeFunc &func = _CAST (NativeFunc &, args[0]);
                        if (func.decl != nullptr)
                          return VAR (func.decl->signature);
  ...
  // tp_exception
  _vm->bind_constructor<-1> (
      _vm->_t (VM::tp_exception), [] (VM *vm, ArgsView args) {
  ...
  _vm->bind (_vm->_t (VM::tp_exception), "__init__(self, msg=...)",
             [] (VM *vm, ArgsView args) {
  ...
  _vm->bind__repr__ (VM::tp_exception, [] (VM *vm, PyObject *obj) {
    ...
    return VAR (
        fmt (obj_type_name (vm, obj->type), '(', self.msg.escape (), ')'));
  });
...
void
add_module_operator (VM *vm)
{
  PyObject *mod = vm->new_module ("operator");
  vm->bind_func<2> (mod, "lt", [] (VM *vm, ArgsView args) {
    return VAR (vm->py_lt (args[0], args[1]));
  });
  vm->bind_func<2> (mod, "le", [] (VM *vm, ArgsView args) {
    return VAR (vm->py_le (args[0], args[1]));
  });
  vm->bind_func<2> (mod, "eq", [] (VM *vm, ArgsView args) {
    return VAR (vm->py_eq (args[0], args[1]));
  });
  vm->bind_func<2> (mod, "ne", [] (VM *vm, ArgsView args) {
    return VAR (vm->py_ne (args[0], args[1]));
  });
  vm->bind_func<2> (mod, "ge", [] (VM *vm, ArgsView args) {
    return VAR (vm->py_ge (args[0], args[1]));
  });
  vm->bind_func<2> (mod, "gt", [] (VM *vm, ArgsView args) {
    return VAR (vm->py_gt (args[0], args[1]));
  });
}
...
struct PyStructTime
{
  ...
  static void
  _register (VM *vm, PyObject *mod, PyObject *type)
  {
...
add_module_time (VM *vm)
{
  ...
  vm->bind_func<0> (mod, "time", [] (VM *vm, ArgsView args) {
    auto now = std::chrono::system_clock::now ();
    return VAR (std::chrono::duration_cast<std::chrono::milliseconds> (
  ...
  vm->bind_func<1> (mod, "sleep", [] (VM *vm, ArgsView args) {
    ...
    while (true)
      {
        auto now = std::chrono::system_clock::now ();
        f64 elapsed = std::chrono::duration_cast<std::chrono::milliseconds> (
                          now - begin)
                          .count ()
                      / 1000.0;
        if (elapsed >= seconds)
...
add_module_math (VM *vm)
{
  ...
  vm->bind_func<1> (mod, "ceil",
                    PK_LAMBDA (VAR ((i64)std::ceil (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "fabs",
                    PK_LAMBDA (VAR (std::fabs (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "floor",
                    PK_LAMBDA (VAR ((i64)std::floor (CAST_F (args[0])))));
  ...
  vm->bind_func<1> (mod, "isfinite",
                    PK_LAMBDA (VAR (std::isfinite (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "isinf",
                    PK_LAMBDA (VAR (std::isinf (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "isnan",
                    PK_LAMBDA (VAR (std::isnan (CAST_F (args[0])))));
  ...
  vm->bind_func<1> (mod, "log2",
                    PK_LAMBDA (VAR (std::log2 (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "log10",
                    PK_LAMBDA (VAR (std::log10 (CAST_F (args[0])))));

  vm->bind_func<2> (
      mod, "pow",
      PK_LAMBDA (VAR (std::pow (CAST_F (args[0]), CAST_F (args[1])))));
  vm->bind_func<1> (mod, "sqrt",
                    PK_LAMBDA (VAR (std::sqrt (CAST_F (args[0])))));

  vm->bind_func<1> (mod, "acos",
                    PK_LAMBDA (VAR (std::acos (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "asin",
                    PK_LAMBDA (VAR (std::asin (CAST_F (args[0])))));
  vm->bind_func<1> (mod, "atan",
                    PK_LAMBDA (VAR (std::atan (CAST_F (args[0])))));
  vm->bind_func<2> (
      mod, "atan2",
      PK_LAMBDA (VAR (std::atan2 (CAST_F (args[0]), CAST_F (args[1])))));
  ...
  vm->bind_func<1> (
      mod, "degrees",
      PK_LAMBDA (VAR (CAST_F (args[0]) * 180 / 3.1415926535897932384)));
  vm->bind_func<1> (
      mod, "radians",
      PK_LAMBDA (VAR (CAST_F (args[0]) * 3.1415926535897932384 / 180)));
...
VM::post_init ()
{
  ...
  _all_types[tp_module].m__getattr__
      = [] (VM *vm, PyObject *obj, StrName name) -> PyObject * {
    const Str &path = CAST (Str &, obj->attr (__path__));
  ...
  bind__eq__ (tp_bound_method, [] (VM *vm, PyObject *lhs, PyObject *rhs) {
    if (!is_non_tagged_type (rhs, vm->tp_bound_method))
      return vm->NotImplemented;
  ...
  for (const char *name :
       { "this", "functools", "heapq", "bisect", "pickle", "_long", "colorsys",
         "typing", "datetime", "dataclasses", "cmath" })
    {
  ...
  try
    {
      CodeObject_ code
          = compile (kPythonLibs["builtins"], "<builtins>", EXEC_MODE);
...
CodeObject_
VM::compile (const Str &source, const Str &filename, CompileMode mode,
             bool unknown_global_scope)
{
