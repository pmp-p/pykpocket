...
template <typename T> struct DoubleLinkedList
{
  ...
  pop_back ()
  {
    ...
    if (empty ())
      throw std::runtime_error (
          "DoubleLinkedList::pop_back() called on empty list");
#endif
  ...
  pop_front ()
  {
    ...
    if (empty ())
      throw std::runtime_error (
          "DoubleLinkedList::pop_front() called on empty list");
#endif
  ...
  back () const
  {
    ...
    if (empty ())
      throw std::runtime_error (
          "DoubleLinkedList::back() called on empty list");
#endif
  ...
  front () const
  {
    ...
    if (empty ())
      throw std::runtime_error (
          "DoubleLinkedList::front() called on empty list");
#endif
  ...
  erase (T *node)
  {
    ...
    if (empty ())
      throw std::runtime_error (
          "DoubleLinkedList::erase() called on empty list");
    ...
    if (n != node)
      throw std::runtime_error (
          "DoubleLinkedList::erase() called on node not in the list");
#endif
    ...
    if (size () == 0)
      {
        if (head.next != &tail || tail.prev != &head)
          {
            throw std::runtime_error ("DoubleLinkedList::size() returned 0 "
...
template <int __BlockSize = 128> struct MemoryPool
{
  static const size_t __MaxBlocks = 256 * 1024 / __BlockSize;
  static const size_t __MinArenaCount
      = PK_GC_MIN_THRESHOLD * 100 / (256 * 1024);
  ...
  struct Arena : LinkedListNode
  {
    ...
    empty () const
    {
      return _free_list_size == 0;
    }
    bool
    full () const
    {
      return _free_list_size == __MaxBlocks;
    }
...
#if PK_DEBUG_MEMORY_POOL
      if (empty ())
        throw std::runtime_error ("Arena::alloc() called on empty arena");
#endif
      _free_list_size--;
...
#if PK_DEBUG_MEMORY_POOL
      if (full ())
        throw std::runtime_error ("Arena::dealloc() called on full arena");
#endif
  ...
  alloc (size_t size)
  {
    ...
    if (_arenas.empty ())
      {
        // std::cout << _arenas.size() << ',' << _empty_arenas.size() << ',' <<
        // _full_arenas.size() << std::endl;
...
#if PK_DEBUG_MEMORY_POOL
    if (p == nullptr)
      throw std::runtime_error ("MemoryPool::dealloc() called on nullptr");
#endif
...
void *
pool64_alloc (size_t size)
{
  return pool64.alloc (size);
}
void
pool64_dealloc (void *p)
{
  pool64.dealloc (p);
}

void *
pool128_alloc (size_t size)
{
  return pool128.alloc (size);
}
void
pool128_dealloc (void *p)
{
  pool128.dealloc (p);
}
