...
struct PyDequeIter // Iterator for the deque type
{
  ...
  PyDequeIter (PyObject *ref, std::deque<PyObject *>::iterator begin,
               std::deque<PyObject *>::iterator end)
      : ref (ref), begin (begin), end (end), current (begin)
  {
    this->is_reversed = false;
  }
  PyDequeIter (PyObject *ref, std::deque<PyObject *>::reverse_iterator rbegin,
...
void
PyDequeIter::_register (VM *vm, PyObject *mod, PyObject *type)
{
...
struct PyDeque
{
  ...
  void
  insertObj (bool front, bool back, int index,
             PyObject *item); // insert at index, used purely for internal
                              // purposes: append, appendleft, insert methods
  PyObject *popObj (bool front, bool back, PyObject *item,
                    VM *vm); // pop at index, used purely for internal
                             // purposes: pop, popleft, remove methods
  int findIndex (VM *vm, PyObject *obj, int start,
                 int stop); // find the index of the given object in the deque
  std::string
  getRepr (VM *vm,
           PyObject *thisObj); // get the string representation of the deque
  // Special methods
  static void _register (VM *vm, PyObject *mod,
                         PyObject *type); // register the type
  void _gc_mark () const; // needed for container types, mark all objects in
                          // the deque for gc
};
void
PyDeque::_register (VM *vm, PyObject *mod, PyObject *type)
{
  ...
  // gets the item at the given index, if index is negative, it will be treated
  // as index + len(deque) if the index is out of range, IndexError will be
  // thrown --> required for [] operator
  vm->bind (type, "__getitem__(self, index) -> PyObject",
            [] (VM *vm, ArgsView args) {
              PyDeque &self = _CAST (PyDeque &, args[0]);
              int index = CAST (int, args[1]);
              index = vm->normalized_index (
                  index, self.dequeItems.size ()); // error is handled by the
                                                   // vm->normalized_index
              return self.dequeItems.at (index);
            });
  // sets the item at the given index, if index is negative, it will be treated
  // as index + len(deque) if the index is out of range, IndexError will be
  // thrown --> required for [] operator
  ...
  // erases the item at the given index, if index is negative, it will be
  // treated as index + len(deque) if the index is out of range, IndexError
  // will be thrown --> required for [] operator
  vm->bind (
      type, "__delitem__(self, index) -> None", [] (VM *vm, ArgsView args) {
        PyDeque &self = _CAST (PyDeque &, args[0]);
        int index = CAST (int, args[1]);
        index = vm->normalized_index (
            index,
            self.dequeItems
                .size ()); // error is handled by the vm->normalized_index
        self.dequeItems.erase (self.dequeItems.begin () + index);
        return vm->None;
      });
  // returns the length of the deque
  vm->bind (type, "__len__(self) -> int", [] (VM *vm, ArgsView args) {
    PyDeque &self = _CAST (PyDeque &, args[0]);
    return VAR (self.dequeItems.size ());
  });
  // returns an iterator for the deque
  vm->bind (
      type, "__iter__(self) -> deque_iterator", [] (VM *vm, ArgsView args) {
        PyDeque &self = _CAST (PyDeque &, args[0]);
        return vm->heap.gcnew<PyDequeIter> (PyDequeIter::_type (vm), args[0],
                                            self.dequeItems.begin (),
                                            self.dequeItems.end ());
      });
  ...
  // returns a string representation of the deque
  vm->bind (type, "__str__(self) -> str", [] (VM *vm, ArgsView args) {
    PyDeque &self = _CAST (PyDeque &, args[0]);
    return VAR (self.getRepr (vm, args[0]));
  });
  // enables comparison between two deques, == and != are supported
  vm->bind (type, "__eq__(self, other) -> bool", [] (VM *vm, ArgsView args) {
    PyDeque &self = _CAST (PyDeque &, args[0]);
  ...
  vm->bind (type, "copy(self) -> deque", [] (VM *vm, ArgsView args) {
    auto _lock = vm->heap.gc_scope_lock (); // locking the heap
    PyDeque &self = _CAST (PyDeque &, args[0]);
    PyObject *newDequeObj = vm->heap.gcnew<PyDeque> (
        PyDeque::_type (vm), vm, vm->None, vm->None); // create the empty deque
    PyDeque &newDeque
        = _CAST (PyDeque &,
                 newDequeObj); // cast it to PyDeque so we can use its methods
    for (auto it = self.dequeItems.begin (); it != self.dequeItems.end ();
         ++it)
  ...
  vm->bind (type, "count(self, obj) -> int", [] (VM *vm, ArgsView args) {
    ...
    for (auto it = self.dequeItems.begin (); it != self.dequeItems.end ();
         ++it)
      {
        if (vm->py_eq ((*it), obj))
          cnt++;
        if (sz != self.dequeItems.size ()) // mutating the deque during
  ...
  vm->bind (type, "index(self, obj, start=None, stop=None) -> int",
            [] (VM *vm, ArgsView args) {
              ...
              if (index != -1)
                ...
                vm->ValueError (_CAST (Str &, vm->py_repr (obj))
                                + " is not in deque");
              return vm->None;
            });
  // NEW: returns the index of the given object in the deque
  ...
  vm->bind (type, "insert(self, index, obj) -> None",
            [] (VM *vm, ArgsView args) {
              ...
              if (self.bounded && self.dequeItems.size () == self.maxlen)
                ...
                self.insertObj (false, false, index,
                                obj); // this index shouldn't be fixed using
                                      // vm->normalized_index, pass as is
              return vm->None;
            });
  ...
  vm->bind_property (
      ...
      [] (VM *vm, ArgsView args) {
        vm->AttributeError ("attribute 'maxlen' of 'collections.deque' "
...
/// @brief initializes a new PyDeque object, actual initialization is done in
/// __init__
...
std::string
PyDeque::getRepr (VM *vm, PyObject *thisObj)
{
  std::stringstream ss;
  ss << "deque([";
  for (auto it = this->dequeItems.begin (); it != this->dequeItems.end ();
       ++it)
    {
      if (*it == thisObj)
        ss << "[...]";
...
int
PyDeque::findIndex (VM *vm, PyObject *obj, int start, int stop)
{
  // the following code is special purpose normalization for this method, taken
  // from CPython: _collectionsmodule.c file
  if (start < 0)
    {
      start = this->dequeItems.size ()
              + start; // try to fix for negative indices
      if (start < 0)
        start = 0;
    }
  if (stop < 0)
    {
      stop
          = this->dequeItems.size () + stop; // try to fix for negative indices
      if (stop < 0)
        stop = 0;
    }
  ...
  PK_ASSERT (start >= 0 && start <= this->dequeItems.size () && stop >= 0
             && stop <= this->dequeItems.size ()
             && start <= stop); // sanity check
  ...
  for (int i = start; i < loopSize; i++)
    {
      if (vm->py_eq (this->dequeItems[i], obj))
        return i;
      if (sz != this->dequeItems.size ()) // mutating the deque during
                                          // iteration is not allowed
        vm->RuntimeError ("deque mutated during iteration");
    }
  return -1;
...
/// @param item if front and back is not set, remove the first occurence of
/// item from the deque
/// @param vm is needed for the py_eq
/// @return PyObject* if front or back is set, this is a pop operation and we
/// return a PyObject*, if front and back are not set, this is a remove
/// operation and we return the removed item or nullptr
PyObject *
PyDeque::popObj (bool front, bool back, PyObject *item, VM *vm)
{
  // error handling
  if (front && back)
    throw std::runtime_error (
        "both front and back are set"); // this should never happen
  if (front || back)
    {
      // front or back is set, we don't care about item, this is a pop
      // operation and we return a PyObject*
      if (this->dequeItems.empty ())
        throw std::runtime_error (
            "pop from an empty deque"); // shouldn't happen
      PyObject *obj;
      if (front)
        {
    ...
    {
      // front and back are not set, we care about item, this is a remove
      // operation and we return the removed item or nullptr
      int sz = this->dequeItems.size ();
      for (auto it = this->dequeItems.begin (); it != this->dequeItems.end ();
           ++it)
        {
          ...
          if (found)
            {
              PyObject *obj
                  = *it; // keep a reference to the object for returning
...
/// @return true if the item was inserted successfully, false if the deque is
/// bounded and is already at its maximum size
void
PyDeque::insertObj (
    bool front, bool back, int index,
    PyObject *item) // assume index is not fixed using the vm->normalized_index
{
  // error handling
  if (front && back)
    throw std::runtime_error (
  ...
  if (front || back)
  ...
  else
    {
      ...
      if (index < 0)
        index = this->dequeItems.size ()
                + index; // try fixing for negative indices
      if (index < 0)     // still negative means insert at the beginning
        this->dequeItems.push_front (item);
      else if (index
               >= this->dequeItems
                      .size ()) // still out of range means insert at the end
        this->dequeItems.push_back (item);
      else
        this->dequeItems.insert ((this->dequeItems.begin () + index),
                                 item); // insert at the given index
    }
}
/// @brief marks the deque items for garbage collection
void
PyDeque::_gc_mark () const
{
