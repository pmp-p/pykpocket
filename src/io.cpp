...
static FILE *
io_fopen (const char *name, const char *mode)
{
#if _MSC_VER
  FILE *fp;
...
static size_t
io_fread (void *buffer, size_t size, size_t count, FILE *fp)
{
#if _MSC_VER
  return fread_s (buffer, std::numeric_limits<size_t>::max (), size, count,
                  fp);
#else
...
unsigned char *
_default_import_handler (const char *name_p, int name_size, int *out_size)
{
#if PK_ENABLE_OS
...
#if PK_ENABLE_OS
void
FileIO::_register (VM *vm, PyObject *mod, PyObject *type)
{
  ...
  vm->bind_method<0> (type, "read", [] (VM *vm, ArgsView args) {
    ...
    // in text mode, CR may be dropped, which may cause `actual_size <
    // buffer_size`
...
FileIO::FileIO (VM *vm, std::string file, std::string mode)
    : file (file), mode (mode)
{
