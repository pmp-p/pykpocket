...
ManagedHeap::sweep ()
{
  ...
  for (auto &[type, count] : deleted)
    {
      std::cout << "GC: " << obj_type_name (vm, type).sv () << "=" << count
                << std::endl;
    }
  std::cout << "GC: " << alive.size () << "/" << gen.size () << " (" << freed
            << " freed)" << std::endl;
  deleted.clear ();
#endif
  gen.clear ();
...
void
FuncDecl::_gc_mark () const
{
  code->_gc_mark ();
  for (int i = 0; i < kwargs.size (); i++)
    PK_OBJ_MARK (kwargs[i].value);
}
