...
struct JsonSerializer
{
  ...
  write_dict (Dict &dict)
  {
    ...
    dict.apply ([&] (PyObject *k, PyObject *v) {
      ...
      if (!is_non_tagged_type (k, vm->tp_str))
        {
          vm->TypeError (fmt ("json keys must be string, got ",
                              obj_type_name (vm, vm->_tp (k))));
        }
  ...
  write_object (PyObject *obj)
  {
    ...
    if (obj == vm->None)
      ...
      {
        vm->TypeError (
            fmt ("unrecognized type ", obj_type_name (vm, obj_t).escape ()));
      }
  }
...
VM::VM (bool enable_os) : heap (this), enable_os (enable_os)
{
  ...
  _import_handler = [] (const char *name_p, int name_size,
                        int *out_size) -> unsigned char * {
    PK_UNUSED (name_p);
    PK_UNUSED (name_size);
    PK_UNUSED (out_size);
    return nullptr;
...
VM::py_str (PyObject *obj)
{
  ...
  if (self != PY_NULL)
    return call_method (self, f);
  return py_repr (obj);
}

PyObject *
VM::py_repr (PyObject *obj)
{
  const PyTypeInfo *ti = _inst_type_info (obj);
  if (ti->m__repr__)
...
PyObject *
VM::find_name_in_mro (Type cls, StrName name)
{
  PyObject *val;
  do
    {
...
bool
VM::isinstance (PyObject *obj, Type base)
{
...
bool
VM::issubclass (Type cls, Type base)
{
  do
    {
      if (cls == base)
...
PyObject *
VM::exec (Str source, Str filename, CompileMode mode, PyObject *_module)
{
  if (_module == nullptr)
    _module = _main;
  try
    ...
    {
      Str msg = "An unknown exception occurred! It could be a bug. Please "
                "report it to @blueloveTH on GitHub.\n";
      _stderr (msg.data, msg.size);
    }
#endif
  callstack.clear ();
...
PyObject *
VM::new_type_object (PyObject *mod, StrName name, Type base,
                     bool subclass_enabled)
{
  ...
  if (!base_info.subclass_enabled)
    {
      TypeError (fmt ("type ", base_info.name.escape (),
                      " is not `subclass_enabled`"));
    }
  PyTypeInfo info{
    obj, base, mod, name, subclass_enabled,
  };
...
Type
VM::_new_type_object (StrName name, Type base, bool subclass_enabled)
{
...
const PyTypeInfo *
VM::_inst_type_info (PyObject *obj)
{
  if (is_int (obj))
    return &_all_types[tp_int];
  if (is_float (obj))
    return &_all_types[tp_float];
  return &_all_types[obj->type];
}

bool
VM::py_eq (PyObject *lhs, PyObject *rhs)
{
  if (lhs == rhs)
    return true;
...
int
VM::normalized_index (int index, int size)
{
  if (index < 0)
    index += size;
  if (index < 0 || index >= size)
    {
      IndexError (std::to_string (index) + " not in [0, "
                  + std::to_string (size) + ")");
    }
  return index;
}
...
PyObject *
VM::py_next (PyObject *obj)
{
  const PyTypeInfo *ti = _inst_type_info (obj);
  if (ti->m__next__)
    return ti->m__next__ (this, obj);
  ...
  if (it == _lazy_modules.end ())
    {
      int out_size;
      unsigned char *out
          = _import_handler (filename.data, filename.size, &out_size);
      if (out == nullptr)
        {
          filename = path.replace ('.', PK_PLATFORM_SEP).str ()
                     + PK_PLATFORM_SEP + "__init__.py";
          is_init = true;
      ...
      if (out == nullptr)
        {
          if (throw_err)
            ImportError (fmt ("module ", path.escape (), " not found"));
          else
            return nullptr;
        }
...
void
VM::parse_int_slice (const Slice &s, int length, int &start, int &stop,
                     int &step)
{
...
PyObject *
VM::_format_string (Str spec, PyObject *obj)
{
  if (spec.empty ())
    return py_str (obj);
  char type;
...
PyObject *
VM::new_module (Str name, Str package)
{
  ...
  if (_modules.contains (name))
    {
      throw std::runtime_error (
          fmt ("module ", name.escape (), " already exists").str ());
    }
...
static std::string
_opcode_argstr (VM *vm, Bytecode byte, const CodeObject *co)
{
  ...
  for (auto byte : co->codes)
    {
      if (byte.op == OP_JUMP_ABSOLUTE || byte.op == OP_POP_JUMP_IF_FALSE
          || byte.op == OP_SHORTCUT_IF_FALSE_OR_POP || byte.op == OP_FOR_ITER)
        {
  ...
  for (int i = 0; i < co->codes.size (); i++)
    {
      ...
      std::string pointer;
      if (std::find (jumpTargets.begin (), jumpTargets.end (), i)
          != jumpTargets.end ())
        {
          pointer = "-> ";
        }
...
void
VM::_log_s_data (const char *title)
{
  if (_main == nullptr)
    return;
  if (callstack.empty ())
  ...
  for (PyObject **p = s_data.begin (); p != s_data.end (); p++)
    {
      ...
      if (obj == nullptr)
        ...
        ss << CAST (f64, obj);
      else if (is_type (obj, tp_str))
        ss << CAST (Str, obj).escape ();
      else if (obj == None)
        ss << "None";
      else if (obj == True)
        ss << "True";
        ...
        ss << "(" << obj_type_name (this, obj->type) << ")";
      ss << ", ";
    }
  ...
  std::cout << output << " " << OP_NAMES[byte.op] << " "
            << _opcode_argstr (nullptr, byte, frame->co) << std::endl;
}
#endif

void
VM::init_builtin_types ()
{
  _all_types.push_back (
      { heap._new<Type> (Type (1), Type (0)), -1, nullptr, "object", true });
  _all_types.push_back (
      { heap._new<Type> (Type (1), Type (1)), 0, nullptr, "type", false });

  if (tp_int != _new_type_object ("int"))
    exit (-3);
  if ((tp_float != _new_type_object ("float")))
    exit (-3);

  if (tp_bool != _new_type_object ("bool"))
    exit (-3);
  if (tp_str != _new_type_object ("str"))
    exit (-3);
  if (tp_list != _new_type_object ("list"))
    exit (-3);
  if (tp_tuple != _new_type_object ("tuple"))
    exit (-3);

  if (tp_slice != _new_type_object ("slice"))
    exit (-3);
  if (tp_range != _new_type_object ("range"))
    exit (-3);
  if (tp_module != _new_type_object ("module"))
    exit (-3);
  if (tp_function != _new_type_object ("function"))
    exit (-3);
  if (tp_native_func != _new_type_object ("native_func"))
    exit (-3);
  if (tp_bound_method != _new_type_object ("bound_method"))
    exit (-3);

  if (tp_super != _new_type_object ("super"))
    exit (-3);
  if (tp_exception != _new_type_object ("Exception", 0, true))
    exit (-3);
  if (tp_bytes != _new_type_object ("bytes"))
    exit (-3);
  if (tp_mappingproxy != _new_type_object ("mappingproxy"))
    exit (-3);
  if (tp_dict != _new_type_object ("dict"))
    exit (-3);
  if (tp_property != _new_type_object ("property"))
    exit (-3);
  if (tp_star_wrapper != _new_type_object ("_star_wrapper"))
    exit (-3);

  if (tp_staticmethod != _new_type_object ("staticmethod"))
    exit (-3);
  if (tp_classmethod != _new_type_object ("classmethod"))
    exit (-3);

  this->None = heap._new<Dummy> (_new_type_object ("NoneType"));
  this->NotImplemented
      = heap._new<Dummy> (_new_type_object ("NotImplementedType"));
  this->Ellipsis = heap._new<Dummy> (_new_type_object ("ellipsis"));
  this->True = heap._new<Dummy> (tp_bool);
  this->False = heap._new<Dummy> (tp_bool);
  this->StopIteration
      = heap._new<Dummy> (_new_type_object ("StopIterationType"));
...
void
VM::_unpack_as_list (ArgsView args, List &list)
{
...
// `heap.gc_scope_lock();` needed before calling this function
void
VM::_unpack_as_dict (ArgsView args, Dict &dict)
{
  for (PyObject *obj : args)
    {
      if (is_non_tagged_type (obj, tp_star_wrapper))
...
void
VM::_prepare_py_call (PyObject **buffer, ArgsView args, ArgsView kwargs,
                      const FuncDecl_ &decl)
{
  ...
  if (args.size () < decl_argc)
    {
      vm->TypeError (fmt (co->name, "() takes ", decl_argc,
                          " positional arguments but ", args.size (),
                          " were given"));
    }
  ...
  if (decl->starred_arg != -1)
  ...
  else
    {
      ...
      if (i < args.size ())
        TypeError (fmt ("too many arguments", " (", decl->code->name, ')'));
    }
  ...
  for (int j = 0; j < kwargs.size (); j += 2)
    {
      ...
      if (index >= 0)
      ...
      else
        {
          ...
          if (vkwargs == nullptr)
            {
              TypeError (fmt (key.escape (),
                              " is an invalid keyword argument for ", co->name,
                              "()"));
            }
          else
            {
...
PyObject *
VM::vectorcall (int ARGC, int KWARGC, bool op_call)
{
  ...
  if (is_non_tagged_type (callable, tp_native_func))
    {
      ...
      if (f.decl != nullptr)
      ...
      else
        {
          if (KWARGC != 0)
            TypeError (
                "old-style native_func does not accept keyword arguments");
      ...
      if (decl->is_simple)
        {
          if (args.size () != decl->args.size ())
            {
              TypeError (fmt (co->name, "() takes ", decl->args.size (),
                              " positional arguments but ", args.size (),
                              " were given"));
            }
          if (!kwargs.empty ())
  ...
  if (is_non_tagged_type (callable, tp_type))
    {
      ...
      PyObject *new_f
          = find_name_in_mro (PK_OBJ_GET (Type, callable), __new__);
      PyObject *obj;
#if PK_DEBUG_EXTRA_CHECK
...
void
VM::delattr (PyObject *_0, StrName _name)
{
          ...
          if (val != nullptr)
            {
              ...
              if (val->type == tp_classmethod)
                return VAR (
                    BoundMethod (obj, PK_OBJ_GET (ClassMethod, val).func));
              return val;
            }
        }
      else
      ...
      if (!is_tagged (cls_var))
        {
          switch (cls_var->type)
            ...
            case tp_classmethod.index:
              return VAR (BoundMethod (
                  _t (objtype), PK_OBJ_GET (ClassMethod, cls_var).func));
            }
        }
      return cls_var;
    }
...
#if PK_USE_PROXY // PYDK
    if ( obj_type_name(vm, objtype).sv() == "ProxyType" )
//  if (OBJ_NAME(_t(obj)) == "ProxyType")
    {
      vm->exec (fmt ("platform.ProxyType.__getproxy(", PK_BITS(obj), ",", name.escape (), ",", __LINE__, ")"));
      return obj->attr ().try_get ("__return");
    }
#endif // PYDK
  if (throw_err)
...
PyObject *
VM::get_unbound_method (PyObject *obj, StrName name, PyObject **self,
                        bool throw_err, bool fallback)
{
  *self = PY_NULL;
  Type objtype (0);
  // handle super() proxy
...
#if PK_USE_PROXY // PYDK
    if ( obj_type_name(vm, objtype).sv() == "ProxyType" )
  //if (OBJ_NAME(objtype) == "ProxyType")
    {
      vm->exec (fmt ("platform.ProxyType.__getproxy(", PK_BITS(obj), ",",
                     name.escape (), ",", __LINE__, ")"));
      return obj->attr ().try_get ("__return");
    }
#endif // PYDK
  if (throw_err)
...
void
VM::setattr (PyObject *obj, StrName name, PyObject *value)
{
  Type objtype (0);
  // handle super() proxy
  if (is_non_tagged_type (obj, tp_super))
  ...
  if (name.sv()[0] != '_')
    if (OBJ_NAME(_t(obj)) == "ProxyType")
      {
        obj->attr().set ("__value", value);
        vm->exec (fmt ("platform.ProxyType.__setattr(", PK_BITS(obj), ",", name.escape (), ",", __LINE__, ")"));
        return;
      }
#endif // PYDK
...
PyObject *
VM::bind (PyObject *obj, const char *sig, NativeFuncC fn, UserData userdata,
          BindType bt)
{
...
PyObject *
VM::bind (PyObject *obj, const char *sig, const char *docstring,
          NativeFuncC fn, UserData userdata, BindType bt)
{
  CodeObject_ co;
  try
    {
...
PyObject *
VM::bind_property (PyObject *obj, Str name, NativeFuncC fget, NativeFuncC fset)
{
...
void
VM::_builtin_error (StrName type)
{
  _error (call (builtins->attr (type)));
}
void
VM::_builtin_error (StrName type, PyObject *arg)
{
  _error (call (builtins->attr (type), arg));
}
void
VM::_builtin_error (StrName type, const Str &msg)
{
  _builtin_error (type, VAR (msg));
}

void
VM::_error (PyObject *e_obj)
{
...
ManagedHeap::mark ()
{
  ...
  for (auto &frame : vm->callstack.data ())
    frame._gc_mark ();
  for (PyObject *obj : vm->s_data)
    PK_OBJ_MARK (obj);
  for (auto [_, co] : vm->_cached_codes)
    co->_gc_mark ();
  if (vm->_last_exception)
    PK_OBJ_MARK (vm->_last_exception);
  if (vm->_curr_class)
    PK_OBJ_MARK (vm->_curr_class);
  if (vm->_c.error != nullptr)
    PK_OBJ_MARK (vm->_c.error);
  if (_gc_marker_ex)
    _gc_marker_ex (vm);
}
...
void
VM::bind__len__ (Type type, i64 (*f) (VM *, PyObject *))
{
  PyObject *obj = _t (type);
...
void
Dict::_probe_0 (PyObject *key, bool &ok, int &i) const
{
  ok = false;
  i64 hash = vm->py_hash (key);
  i = hash & _mask;
  // std::cout << CAST(Str, vm->py_repr(key)) << " " << hash << " " << i <<
  // std::endl;
  for (int j = 0; j < _capacity; j++)
    {
      ...
      i = ((5 * i) + 1) & _mask;
      // std::cout << CAST(Str, vm->py_repr(key)) << " next: " << i <<
      // std::endl;
    }
}

void
Dict::_probe_1 (PyObject *key, bool &ok, int &i) const
{
  ok = false;
...
void
NativeFunc::check_size (VM *vm, ArgsView args) const
{
  if (args.size () != argc && argc != -1)
    {
      vm->TypeError (
          fmt ("expected ", argc, " arguments, got ", args.size ()));
    }
}

PyObject *
NativeFunc::call (VM *vm, ArgsView args) const
{
  return f (vm, args);
}
