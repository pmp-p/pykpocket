...
CodeObject_
Compiler::push_global_context ()
{
  CodeObject_ co
      = std::make_shared<CodeObject> (lexer->src, lexer->src->filename);
  contexts.push (CodeEmitContext (vm, co, contexts.size ()));
  return co;
}

FuncDecl_
Compiler::push_f_context (Str name)
{
...
Compiler::pop_context ()
{
  ...
  // however, this is buggy...since there may be a jump to the end (out of
  // bound) even if the last opcode is a return
  ...
  if (codes.size () > 65535 && ctx ()->co->src->mode != JSON_MODE)
    {
      // json mode does not contain jump instructions, so it is safe to ignore
      // this check
...
#define PK_NO_INFIX nullptr, PREC_NONE
  for (TokenIndex i = 0; i < kTokenCount; i++)
    rules[i] = { nullptr, PK_NO_INFIX };
  rules[TK (".")] = { nullptr, PK_METHOD (exprAttrib), PREC_ATTRIB };
  rules[TK ("(")] = { PK_METHOD (exprGroup), PK_METHOD (exprCall), PREC_CALL };
  rules[TK ("[")]
      = { PK_METHOD (exprList), PK_METHOD (exprSubscr), PREC_SUBSCRIPT };
  ...
  rules[TK ("-")]
      = { PK_METHOD (exprUnaryOp), PK_METHOD (exprBinaryOp), PREC_TERM };
  rules[TK ("*")]
      = { PK_METHOD (exprUnaryOp), PK_METHOD (exprBinaryOp), PREC_FACTOR };
  rules[TK ("~")] = { PK_METHOD (exprUnaryOp), nullptr, PREC_UNARY };
  rules[TK ("/")] = { nullptr, PK_METHOD (exprBinaryOp), PREC_FACTOR };
  rules[TK ("//")] = { nullptr, PK_METHOD (exprBinaryOp), PREC_FACTOR };
  rules[TK ("**")]
      = { PK_METHOD (exprUnaryOp), PK_METHOD (exprBinaryOp), PREC_EXPONENT };
  ...
  rules[TK ("not in")]
      = { nullptr, PK_METHOD (exprBinaryOp), PREC_COMPARISION };
  rules[TK ("is not")]
      = { nullptr, PK_METHOD (exprBinaryOp), PREC_COMPARISION };
...
bool
Compiler::match (TokenIndex expected)
{
...
void
Compiler::consume (TokenIndex expected)
{
  if (!match (expected))
    {
      SyntaxError (fmt ("expected '", TK_STR (expected), "', got '",
                        TK_STR (curr ().type), "'"));
    }
}

bool
Compiler::match_newlines_repl ()
{
  return match_newlines (mode () == REPL_MODE);
}

bool
Compiler::match_newlines (bool repl_throw)
{
  bool consumed = false;
  if (curr ().type == TK ("@eol"))
    {
...
void
Compiler::EXPR (bool push_stack)
{
  parse_expression (PREC_TUPLE + 1, push_stack);
}

void
Compiler::EXPR_TUPLE (bool push_stack)
{
  ...
  do
    {
      ...
      if (items.size () == 1 && match (TK ("for")))
        {
          if (parsing_dict)
            _consume_comp<DictCompExpr> (std::move (items[0]));
          else
            _consume_comp<SetCompExpr> (std::move (items[0]));
          consume (TK ("}"));
          return;
        }
      match_newlines_repl ();
...
void
Compiler::exprLiteral0 ()
{
  ctx ()->s_expr.push (make_expr<Literal0Expr> (prev ().type));
}

void
Compiler::compile_block_body (void (Compiler::*callback) ())
{
  if (callback == nullptr)
...
void
Compiler::compile_normal_import ()
{
  do
    {
      consume (TK ("@id"));
      Str name = prev ().str ();
      ctx ()->emit_ (OP_IMPORT_PATH, ctx ()->add_const_string (name.sv ()),
                     prev ().line);
      if (match (TK ("as")))
        {
          consume (TK ("@id"));
...
__EAT_DOTS_END:
  ...
  ctx ()->emit_ (OP_IMPORT_PATH, ctx ()->add_const_string (ss.str ().sv ()),
                 prev ().line);
  consume (TK ("import"));
  ...
  if (match (TK ("*")))
    {
      if (name_scope () != NAME_GLOBAL)
        SyntaxError (
            "from <module> import * can only be used in global scope");
...
void
Compiler::parse_expression (int precedence, bool push_stack)
{
  PrattCallback prefix = rules[curr ().type].prefix;
  if (prefix == nullptr)
...
Compiler::compile_if_stmt ()
{
  ...
  if (match (TK ("elif")))
    {
      int exit_patch
          = ctx ()->emit_ (OP_JUMP_ABSOLUTE, BC_NOARG, prev ().line);
...
Compiler::compile_for_loop ()
{
  ...
  if (!ok)
    SyntaxError (); // this error occurs in `vars` instead of this line,
                    // but...nevermind
  compile_block_body ();
  ctx ()->emit_ (OP_LOOP_CONTINUE, ctx ()->get_loop (), BC_KEEPLINE);
  ctx ()->exit_block ();
...
Compiler::compile_try_except ()
{
      ...
      do
        {
          ...
          int patch
              = ctx ()->emit_ (OP_POP_JUMP_IF_FALSE, BC_NOARG, BC_KEEPLINE);
          // on match
          if (!as_name.empty ())
            {
              ctx ()->emit_ (OP_DUP_TOP, BC_NOARG, BC_KEEPLINE);
  ...
  if (finally_entry != -1)
    {
      ctx ()->emit_ (OP_LOAD_INTEGER, (uint16_t)ctx ()->co->codes.size () + 2,
                     BC_KEEPLINE);
...
bool
Compiler::try_compile_assignment ()
{
  switch (curr ().type)
    {
    case TK ("+="):
...
Compiler::compile_stmt ()
{
  ...
  switch (prev ().type)
    ...
    case TK ("++"):
      {
        ...
        switch (scope)
          {
          case NAME_LOCAL:
            ctx ()->emit_ (OP_INC_FAST, ctx ()->add_varname (name),
                           prev ().line);
            break;
          case NAME_GLOBAL:
    ...
    case TK ("--"):
      {
        ...
        switch (name_scope ())
          {
          case NAME_LOCAL:
            ctx ()->emit_ (OP_DEC_FAST, ctx ()->add_varname (name),
                           prev ().line);
            break;
          case NAME_GLOBAL:
    ...
    case TK ("=="):
      {
        ...
        consume (TK ("=="));
        if (!ok)
          SyntaxError ("label " + prev ().str ().escape ()
                       + " already exists");
        consume_end_stmt ();
      }
      break;
    ...
    default:
      {
        advance (-1); // do revert since we have pre-called advance() at the
                      // beginning
        EXPR_TUPLE ();
                ...
                if (ctx ()->is_compiling_class)
                  {
                    ...
                    NameExpr *ne = static_cast<NameExpr *> (
                        ctx ()->s_expr.top ().get ());
                    ctx ()->emit_ (OP_ADD_CLASS_ANNOTATION, ne->name.index,
                                   BC_KEEPLINE);
                  }
              }
          }
...
void
Compiler::_add_decorators (const std::vector<Expr_> &decorators)
{
  // [obj]
...
void
Compiler::compile_class (const std::vector<Expr_> &decorators)
{
  consume (TK ("@id"));
...
void
Compiler::_compile_f_args (FuncDecl_ decl, bool enable_type_hints)
{
  ...
  do
    {
      ...
      if (decl->starred_arg != -1
          && decl->code->varnames[decl->starred_arg] == name)
        {
          SyntaxError ("duplicate argument name");
        }
      if (decl->starred_kwarg != -1
          && decl->code->varnames[decl->starred_kwarg] == name)
        {
          SyntaxError ("duplicate argument name");
        }
...
void
Compiler::compile_function (const std::vector<Expr_> &decorators)
{
  ...
  if (decl->code->codes.size () >= 2
      && decl->code->codes[0].op == OP_LOAD_CONST
...
PyObject *
Compiler::to_object (const TokenValue &value)
{
...
Compiler::Compiler (VM *vm, const Str &source, const Str &filename,
                    CompileMode mode, bool unknown_global_scope)
{
  this->vm = vm;
  this->used = false;
...
// TODO: refactor this
void
Lexer::throw_err (StrName type, Str msg, int lineno, const char *cursor)
{
