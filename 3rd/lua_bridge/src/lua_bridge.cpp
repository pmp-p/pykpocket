...
#define LUA_PROTECTED(__B)                                                    \
  {                                                                           \
    LuaExceptionGuard __guard;                                                \
    __B;                                                                      \
  }
...
struct PyLuaTable : PyLuaObject
{
  ...
  _register (VM *vm, PyObject *mod, PyObject *type)
  {
    ...
    ti->m__getattr__ = [] (VM *vm, PyObject *obj, StrName name) {
      ...
      LUA_PROTECTED (lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r);
                     lua_pushstring (_L, std::string (name.sv ()).c_str ());
                     lua_gettable (_L, -2);
                     PyObject *ret = lua_popx_to_python (vm); lua_pop (_L, 1);
                     return ret;)
    };

    ti->m__setattr__ = [] (VM *vm, PyObject *obj, StrName name,
                           PyObject *val) {
      const PyLuaTable &self = _CAST (PyLuaTable &, obj);
      LUA_PROTECTED (lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r);
                     lua_pushstring (_L, std::string (name.sv ()).c_str ());
                     lua_push_from_python (vm, val); lua_settable (_L, -3);
                     lua_pop (_L, 1);)
    };
    ...
    vm->bind__getitem__ (t, [] (VM *vm, PyObject *obj, PyObject *key) {
      const PyLuaTable &self = _CAST (PyLuaTable &, obj);
      LUA_PROTECTED (lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r);
                     lua_push_from_python (vm, key); lua_gettable (_L, -2);
                     PyObject *ret = lua_popx_to_python (vm); lua_pop (_L, 1);
                     return ret;)
    });

    vm->bind__setitem__ (
        t, [] (VM *vm, PyObject *obj, PyObject *key, PyObject *val) {
          const PyLuaTable &self = _CAST (PyLuaTable &, obj);
          LUA_PROTECTED (lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r);
    ...
    vm->bind__delitem__ (t, [] (VM *vm, PyObject *obj, PyObject *key) {
      const PyLuaTable &self = _CAST (PyLuaTable &, obj);
      LUA_PROTECTED (lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r);
                     lua_push_from_python (vm, key); lua_pushnil (_L);
                     lua_settable (_L, -3); lua_pop (_L, 1);)
    });
    ...
    vm->bind__contains__ (t, [] (VM *vm, PyObject *obj, PyObject *key) {
      ...
      LUA_PROTECTED (lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r);
                     lua_push_from_python (vm, key); lua_gettable (_L, -2);
                     bool ret = lua_isnil (_L, -1) == 0; lua_pop (_L, 2);
    ...
    vm->bind (type, "keys(self) -> list", [] (VM *vm, ArgsView args) {
      ...
      LUA_PROTECTED (
          lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r); List ret; table_apply (
              vm, [&] (PyObject *key, PyObject *val) { ret.push_back (key); });
          lua_pop (_L, 1); return VAR (std::move (ret));)
    });
    ...
    vm->bind (type, "values(self) -> list", [] (VM *vm, ArgsView args) {
      ...
      LUA_PROTECTED (
          lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r); List ret; table_apply (
              vm, [&] (PyObject *key, PyObject *val) { ret.push_back (val); });
          lua_pop (_L, 1); return VAR (std::move (ret));)
    });
    ...
    vm->bind (type, "items(self) -> list[tuple]", [] (VM *vm, ArgsView args) {
      const PyLuaTable &self = _CAST (PyLuaTable &, args[0]);
      LUA_PROTECTED (
          lua_rawgeti (_L, LUA_REGISTRYINDEX, self.r); List ret;
          table_apply (vm,
...
static PyObject *
lua_popx_multi_to_python (VM *vm, int count)
{
  if (count == 0)
    {
      return vm->None;
...
struct PyLuaFunction : PyLuaObject
{
  ...
  _register (VM *vm, PyObject *mod, PyObject *type)
  {
    ...
    vm->bind_method<-1> (type, "__call__", [] (VM *vm, ArgsView args) {
      if (args.size () < 1)
        vm->TypeError ("__call__ takes at least 1 argument");
...
void
lua_push_from_python (VM *vm, PyObject *val)
{
  if (val == vm->None)
    {
      lua_pushnil (_L);
  ...
  vm->RuntimeError (
      fmt ("unsupported python type: ", obj_type_name (vm, t).escape ()));
}

PyObject *
lua_popx_to_python (VM *vm)
{
  ...
  switch (type)
    ...
    case LUA_TFUNCTION:
      {
        PyObject *obj
            = vm->heap.gcnew<PyLuaFunction> (PyLuaFunction::_type (vm));
        return obj;
      }
    default:
      {
...
void
initialize_lua_bridge (VM *vm, lua_State *newL)
{
