...
static PyObject *convert_cjson_to_python_object (const cJSON *const item,
                                                 VM *vm);

template <typename T>
static cJSON *
convert_list_to_cjson (const T &list, VM *vm)
{
  cJSON *cjson_list = cJSON_CreateArray ();
  for (auto &element : list)
    {
      cJSON_AddItemToArray (cjson_list,
                            convert_python_object_to_cjson (element, vm));
    }
  return cjson_list;
}

static cJSON *
covert_dict_to_cjson (const Dict &dict, VM *vm)
{
  cJSON *cjson_object = cJSON_CreateObject ();
  dict.apply ([&] (PyObject *key, PyObject *val) {
    cJSON_AddItemToObject (cjson_object, CAST (Str &, key).c_str (),
                           convert_python_object_to_cjson (val, vm));
  });
  return cjson_object;
}

static cJSON *
convert_python_object_to_cjson (PyObject *obj, VM *vm)
{
  if (obj == vm->None)
  ...
  switch (obj_t)
    ...
    case VM::tp_int.index:
      cJSON_CreateNumber (_CAST (i64, obj));
    case VM::tp_float.index:
      cJSON_CreateNumber (_CAST (f64, obj));
    case VM::tp_bool.index:
      cJSON_CreateBool (obj == vm->True);
    case VM::tp_str.index:
      cJSON_CreateString (_CAST (Str &, obj).c_str ());
...
static PyObject *
convert_cjson_to_list (const cJSON *const item, VM *vm)
{
  List output;
...
static PyObject *
convert_cjson_to_dict (const cJSON *const item, VM *vm)
{
  Dict output (vm);
...
static PyObject *
convert_cjson_to_python_object (const cJSON *const item, VM *vm)
{
